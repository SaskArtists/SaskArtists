...
ContentFlow.prototype = {
	...
	_init: function () {
		...
		$CF(this.Container)
			.addClassName("ContentFlow");
		var A = $CF(this.Container)
			.getChildrenByClassName("flow")[0];
		if (!A) {
	...
	_initAddOns: function () {
		...
		if (this._userConf.useAddOns) {
			...
			if (this.Container.getAttribute("useAddOns")) {
				C = this.Container.getAttribute("useAddOns")
					.split(" ")
			} else {
